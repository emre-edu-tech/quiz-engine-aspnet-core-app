...
| **Save vs. Publish** | - Save: Persists all question/choice data to the database. IsPublished remains false. <br> - Publish: Generates a unique Slug (GUID), sets IsPublished = true, and returns the full public URL. The slug is generated only once on first publish and never regenerated on subsequent saves. |